                    ...
                    <div className='d-flex gap-2'>
                      ...
                      <div className='col-6 p-4'>
                          { permessi.seeContabile == true && (<div style={style} className='bg-primary text-white p-3 rounded-4'>
                               { course.tipo == "stock" && (contabile.stock < 10 && contabile.stock > 0) && <div className='bg-warning fs-4 p-3 rounded-3 m-2'>Stock in Esaurimento</div>} 
                                 { course.tipo == "stock" &&  contabile.stock === 0 && <div className='bg-danger fs-4 p-3 rounded-3 m-2'>Stock Esaurito</div>} 
                                  <h2>Entrate {contabile.totale_entrate} €</h2>
                                  <h2>Uscite {contabile.totale_uscite} €</h2>
                                  <h2>Profit {contabile.totale_profit} €</h2>
                                  <hr />
                                  <div className='d-flex gap-4 fs-5'>
                          {  course.tipo == "stock"  &&(<div className="d-flex flex-column gap-4"> <h4>Stock:{contabile.stock}</h4>
                                    <h4>Costo:{contabile.costo} €</h4></div>)}
                                    <h4>Venduti:{contabile.venduti}</h4>
                                  </div>
                                  { course.tipo == "stock" && permessi.addStock == true && (<div style={{cursor:'pointer'}} onClick={redirec} className='bg-warning p-1 rounded-5 mt-5 d-flex gap-5 w-25 h-100 align-items-center justify-content-center'>
