...
const EditWorkers = ({id}:{id:any}) => {
  ...
  const handleSubmit = async (e:any) => {
    e.preventDefault();
    try {
      const response = await axios.put(`${apiURL}/modifica/workers/${id}`, formData);
      console.log(response)
      window.location.href =  `/scheda/workers/${id}`;
    } catch (error) {
      console.error('Errore durante l\'invio dei dati:', error);
    }
  };
  ...
  return (
    <div className='d-flex '>
        <div  style={divStyle}  className='bg-primary rounded-4'> 
              ...
              { type == "workers" && <EditWorkers id={id}/>}
          </div>
    </div>
